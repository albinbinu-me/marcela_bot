...
@pytest.mark.asyncio
async def test_rest_require_admin_owner_allows_creator(mock_config):
    chat_iid_value = PydanticObjectId("507f1f77bcf86cd799439012")
    user = MagicMock()
    user.tid = 555

    admin_member = MagicMock()
    admin_member.status = ChatMemberStatus.CREATOR
    admin_member.can_restrict_members = False

    admin_record = MagicMock()
    admin_record.member = admin_member
...
@pytest.mark.asyncio
async def test_rest_require_admin_owner_rejects_non_creator(mock_config):
    chat_iid_value = PydanticObjectId("507f1f77bcf86cd799439013")
    user = MagicMock()
    user.tid = 777

    admin_member = MagicMock()
    admin_member.status = ChatMemberStatus.ADMINISTRATOR
    admin_member.can_restrict_members = True
...
@pytest.mark.asyncio
async def test_rest_require_admin_owner_bypasses_permission_check(mock_config):
    chat_iid_value = PydanticObjectId("507f1f77bcf86cd799439014")
    user = MagicMock()
    user.tid = 888

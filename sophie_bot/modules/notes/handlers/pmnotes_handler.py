...
from sophie_bot.modules.notes.callbacks import PrivateNotesStartUrlCallback, PrivateNoteStartUrlCallback
...
from sophie_bot.modules.notes.handlers.get import GetNote
...
class PrivateNoteConnectHandler(SophieMessageHandler):
    @staticmethod
    def filters() -> tuple[CallbackType, ...]:
        return (PrivateNoteStartUrlCallback.filter(),)
    ...
    async def handle(self) -> Any:
        ...
        user_id = self.event.from_user.id
        command_start: PrivateNoteStartUrlCallback = self.data["command_start"]
        chat_id = command_start.chat_id
        note_name = command_start.note_name
        ...
        # Inject notename into self.data manually for GetNote
        self.data["notename"] = note_name

...
class DeleteMuteUserHandler(MuteUserHandler):
    ...
    async def handle(self) -> Any:
        ...
        if not self.event.reply_to_message:
            return await self.event.reply(_("Reply a message to send perfome mute"))
        ...
        if explicit_reason:
            doc = Section(
                ...
                KeyValue(_("User"), UserLink(user.chat_id, user.first_name)),
                KeyValue(_("Muted by"), UserLink(self.event.from_user.id, self.event.from_user.first_name)),
                KeyValue(_("Reason"), reason),
                title=_("User muted"),
            )
            await self.event.reply(str(doc))
        else:
            await self.event.reply(_("User muted and deleted his message"))

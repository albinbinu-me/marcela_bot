...
from stfu_tg import Code, HList, Italic, VList
...
def format_cmd_args(arguments: dict[str, ArgFabric]) -> HList:
    formatted_descriptions = (f"<{arg.description}>" for arg in arguments.values())
...
def format_handler(handler: HandlerHelp, show_only_in_groups: bool = True, show_disable_able: bool = True):
    ...
    flags = []
    if show_only_in_groups and handler.only_chats:
        flags.append(str(_("Only in groups")))
    if show_disable_able and handler.disableable:
        flags.append(str(_("Disable-able")))

    flag_node = Italic(f" ({', '.join(flags)})") if flags else None
    desc_node = f" - {handler.description}" if handler.description else ""

    return HList(cmd_and_args, desc_node, flag_node)

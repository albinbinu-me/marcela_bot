...
@flags.help(
    description=l_("Connects to the current chat."),
...
class ConnectGroupCmd(SophieMessageHandler):
    ...
    async def handle(self):
        ...
        user_db = await ChatModel.get_by_tid(user_id)
        if not user_db:
            return

        # Check permissions
        if not await check_connection_permissions(self.connection.db_model.iid, user_db.iid):
        ...
        try:
            ...
            await self.event.reply(
                _("You are connected! Check your DM with Macela."),
            ...
            await self.event.reply(
                _(
                    "You must start the bot in DM first to connect. Once you initialize connection with Macela, you'll be connected."
                ),
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text=_("Start Macela"), url=f"https://t.me/{CONFIG.username}?start=connect_{chat_id}"
                            )
                        ]
                    ]
                ),

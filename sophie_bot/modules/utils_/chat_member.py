...
async def update_chat_members(chat: ChatModel):
    ...
    for member in chat_members:
        ...
        if not user:
            # Dynamic Upsert integration: forcefully instantiate unaccounted Telegram admins
            user = await ChatModel.upsert_user(member.user)
            if not user:
                log.debug("user_details: user not found in database and upsert failed", user_id=member.user.id)
                continue

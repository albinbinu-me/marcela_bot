...
@dataclass
class SophieMetrics:
    """Container for all Macela Bot metrics"""
...
def make_metrics(registry: CollectorRegistry, config: Any) -> SophieMetrics:
    """Create all Macela Bot metrics and register them with the registry"""

    # Create metrics
    updates_total = Counter(
        "macela_updates_total",
    ...
    messages_total = Counter(
        "macela_messages_total", "Total number of messages processed", ["message_kind"], registry=registry
    )

    handler_errors_total = Counter(
        "macela_handler_errors_total", "Total number of handler errors", ["handler", "exception"], registry=registry
    )

    external_errors_total = Counter(
        "macela_external_errors_total",
    ...
    inflight_handlers = Gauge(
        "macela_inflight_handlers", "Number of handlers currently being processed", registry=registry
    )

    event_loop_lag_seconds = Gauge("macela_event_loop_lag_seconds", "Event loop lag in seconds", registry=registry)

    # Create histogram buckets
    buckets = METRICS_HISTOGRAM_BUCKETS

    handler_duration_seconds = Histogram(
        "macela_handler_duration_seconds",
    ...
    external_request_duration_seconds = Histogram(
        "macela_external_request_duration_seconds",
    ...
    build_info = Info("macela_build_info", "Build information", registry=registry)

    # AI-specific metrics
    ai_requests_total = Counter(
        "macela_ai_requests_total",
    ...
    ai_active_conversations = Gauge(
        "macela_ai_active_conversations",
        "Number of active AI conversations",
        registry=registry,
    )

    ai_request_duration_seconds = Histogram(
        "macela_ai_request_duration_seconds",
    ...
    ai_tool_duration_seconds = Histogram(
        "macela_ai_tool_duration_seconds",
